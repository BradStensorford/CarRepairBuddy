...
var shopServiceOptions = {
    valueNames: ['iname', 'bowner', 'be-add', { attr: 'data-id', name: 'selectId' }],
    item: '<li class="media list-group-item"><div class="media-body"><h4 class="media-heading"><strong class="iname"></strong></h4> <span class="label label-warning">Business Owner:  <i class="bowner"></i></span> <br> <span class="label label-primary">Email Address: <i class="be-add"></i></span> <hr style="margin-top: 5px;"> <button class="btn btn-primary btn-sm pull-right selectId" onclick="modalShowData(this)">More Information</button></div></li>',
    page: 5,
    pagination: true
};
...
dbRef.child("ShopOverallServices").on('value', function(snapshot) {
    ...
    for (var i = 0; i <= stack.length - 1; i++) {
        ...
        var iown  = stack[i].val().serviceOwner;
        ...
        dbRef.child("Users").child(iown).on('value', function(snapshot){
            if (snapshot.val() != null) {
                ...
                serviceList.add({
                    'iname': iname,
                    'bowner': user.users_firstName + " " + user.users_middleName + " " + user.users_lastName,
                    'be-add': user.users_emailAdd,
                    'selectId': id
                });
            }
        });
...
function modalShowData(itemid) {
    ...
    $("#showDataModal").on('shown.bs.modal', function(e) {
        dbRef.child("ShopOverallServices").child(id).on('value', function(snapshot) {
            $("#serviceName").val(snapshot.val().serviceName);

            var itemOwner = snapshot.val().serviceOwner;
            $("#feedbackServiceName").val(snapshot.val().serviceName);
...
function serviceFeedbackButton() {
    ...
    dbRef.child("DriversFeedbackItem").child(myId).push().set({
        ...
        feedbackDatabase: "ShopOverallServices",
