...
dbRef.child("OwnersFeedbackItem").child(getId).on('value', function(snapshot) {
    ...
    snapshot.forEach(function(childSnapshot) {
        ...
        var iown = childSnapshot.val().feedbackBy;
        var imes = childSnapshot.val().feedbackMessage;
        var tmstmp = childSnapshot.val().feedbackSent;
        var byId = childSnapshot.val().feedbackBy;
        
        dbRef.child("Users").child(iown).once("value", function(snapshot) {
            var user = snapshot.val();
            userList.add({
                'iname': iname,
                'iown': user.users_firstName + " " + user.users_lastName,
                'imes': imes,
                'itmp': new Date(tmstmp).toDateString() + " " + new Date(tmstmp).toLocaleTimeString(),
                'byId': byId
            });
        });
    });
