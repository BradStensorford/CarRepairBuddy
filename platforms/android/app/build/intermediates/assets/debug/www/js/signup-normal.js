...
$("#formUpDataNormal").bootstrapValidator({
    ...
    var df5 = getFormData($form)["inputContactNumber"];
    var df6 = getFormData($form)["inputEmail"];
    var df7 = getFormData($form)["inputHomeAddress"];
            ...
            taskUpload.on('state_changed', function(snapshot) {
                ...
                taskUpload.snapshot.ref.getDownloadURL().then(function(downloadURL) {
                    dbRef.child("Users").child(userCurrent.uid).set({
                        ...
                        users_emailAdd: df6.replace("%40", "@").toString().trim(),
                        users_homeAdd: df7.replace("+", " ").toString().trim(),
                        users_conNum: df5.toString().trim(),
                        users_type: 2,
