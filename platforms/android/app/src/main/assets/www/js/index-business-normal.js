...
for (var i = 0; i < objectResult.length; i++) {
    var ref = JSON.parse(window.localStorage.getItem("CACHE_" + objectResult[i].business_name));
    hackerList.add({
        'business_name': objectResult[i].business_name,
        'business_address': objectResult[i].business_address + " [Distance " + ref.distance.toFixed(2) + "km ]",
