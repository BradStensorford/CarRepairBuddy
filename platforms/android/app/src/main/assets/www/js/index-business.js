    ...
    if (typeof geoQuery !== "undefined") {
        ...
        if (!navigator.onLine) {
            ...
            for (var i = 0; i < keys.length; i++) {
                var key = keys[i];
                if (key.indexOf("CACHE_") !== -1) {
                    var ref = JSON.parse(localStorage.getItem(key));
                    var typesIndicator = ref.business_types.toString();
                    var name_owner = ref.business_name;
                    map.addMarker({
                        position: { lat: ref.location[0], lng: ref.location[1] },
                        icon: 'red',
                        title: name_owner,
                        snippet: ref.distance.toFixed(2) + "km distance from your location.",
                        item: ref.snapshot_key
                    }, function(marker) {
        ...
        geoQuery.on("key_entered", function(key, location, distance) {
            ...
            dbRef.child("BusinessStores").child(key).on('value', function(snapshot) {
                ...
                var reference_object = snapshot.val();
                reference_object.distance = distance;
                reference_object.location = location;
                reference_object.snapshot_key = snapshot.key;
                reference_object.lastUpdate = Date.now();
                localStorage.setItem("CACHE_" + name_owner, JSON.stringify(reference_object));
                //console.log(name_owner + " " + JSON.stringify(reference_object));
                map.addMarker({
